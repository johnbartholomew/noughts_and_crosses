...
fn main() {
    ...
    let result = match solve(Board::new(), &mut games) {
        Status::Win => "win for the first player",
        Status::Draw => "draw",
        Status::Loss => "win for the second player",
    };
...
fn solve(board: Board, games: &mut u32) -> Status {
    if !board.has_moves() {
        *games += 1;
        return Status::Draw;
    }
